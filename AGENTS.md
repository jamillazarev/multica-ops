...
7. **`bash scripts/check-releases.sh`** — a published release's notes are a snapshot taken once,
   so a marked correction added to a frozen changelog entry afterwards reaches the file and never
   the page most people read. This compares every release against its entry and prints the one
   command each gap needs (`--emit <dir>` writes the files). It is a report, not a gate: it needs
   the network and `gh`, and preflight runs offline. Publishing stays the owner's hand.
8. **Regenerate and push the docs site** — `python3 scripts/generate.py` in the `ai` repo, then push. The site deploys continuously, so a skipped regen silently ships the previous pages against the new tag. (a previous release's site lagged its tag the same way.)
