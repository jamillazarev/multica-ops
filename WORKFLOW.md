...
```mermaid
flowchart TD
    U["Any first message —<br/>even a bare /multica-ops:mops or 'hi'"] --> DZ["Day zero: installed · current ·<br/>signed in · workspace · daemon · runtimes<br/><i>one ladder with its fixes, not six prompts</i>"]
    DZ --> Q{"Three questions:<br/>what exists · what you want ·<br/>who runs the work"}
    U -->|"just a question,<br/>nothing to build"| CONSULT["/multica-ops:consult — advise,<br/>zero footprint · no machinery<br/>unless the answer leads there"]
    CONSULT -.->|"'let's build it'<br/>seeds a project"| Q
    Q -->|"nothing, want a team"| SHAPE
    Q -->|"a workspace already"| JOIN["/multica-ops:join — audit, then fix<br/>in approved batches"]
    Q -->|"a backlog elsewhere"| IMP["/multica-ops:import — mapping shown first,<br/>issues created unassigned"]
...
> Nobody is asked to choose a command. Crew mode is the **default offer after `/multica-ops:import`
...
```mermaid
flowchart LR
    subgraph INIT["/multica-ops:init — a company"]
        I1["shaping → interview"] --> I2["conductor + squads"] --> I3["roadmap · ICE · discovery"]
    end
    subgraph CREW["/multica-ops:mops crew — a crew"]
    ...
    subgraph JOIN["/multica-ops:join — inherit"]
...
```mermaid
flowchart TD
    GO["/multica-ops:upgrade"] --> L1{"Is this skill's copy<br/>on your machine current?"}
    ...
    IDLE -->|"no"| WAIT["Say what's in flight.<br/>Wait for idle, or /multica-ops:mops stop<br/>if the owner accepts it"]
    WAIT --> IDLE
