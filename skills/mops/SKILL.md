...
already neutral (`/multica-ops:ship` is a release, a batch, an episode); the failure is examples and
...
`/multica-ops:mops measure` miss is a result). The scoreboard is the product and its metrics, never the
owner's mood.

**The guide is curated, not autogenerated.** Agents propose changes to the shared guide;
Mops or the owner decides and writes them, in batches — a published comparison (coding-agent
benchmarks) found repo context files don't lift success rates while adding ~20% inference cost,
LLM-written ones slightly worse — sources/SOURCES.md; batching also guards the cached prefix from churn.
