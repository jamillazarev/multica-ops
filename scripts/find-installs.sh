...
# the trap: git prints `Aborting` and `Updating <old>..<new>` on adjacent lines, so a glance reads
...
for key,entries in d.get("plugins",{}).items():
  ...
  # The SCOPE is carried into the route: `claude plugin update <p>@<m>` acts on USER scope and
  # reports "already at the latest version" while a PROJECT-scope install of the same plugin sits a
  # release behind. Measured next door 2026-08-16, right after a release.
...
if [ -e "$adir" ]; then
  v=$(read_version "$adir")
  # Ask, do not guess. "it usually is a clone" was true on 2026-08-01 and is the sentence a reader
  # skims past; the two routes are mutually destructive, so the row states which one applies here.
