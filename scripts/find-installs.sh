...
if [ -e "$adir" ]; then
  v=$(read_version "$adir")
  add "$adir" "plugin, Antigravity" "git pull --ff-only in place if it is a clone (it usually is — checked 2026-08-01), else re-copy the source or agy plugin install <url>" "$(clone_state "$adir")" "$v"
fi
