...
For calibration: the shipped `GUIDE-template.md` is **~2.9k tokens** (measure yours — it grows, and this figure has moved three times already), a median community skill
...
what makes an owner ask "why so many?", and by the time `/multica-ops:audit` notices, the team is built
...
fit, you hire the missing person.** `/multica-ops:audit` reports load per agent and names the split
candidates.
...
`theatre: personas · axis: …` as its description's first line, is **excluded from `/multica-ops:mops team`
