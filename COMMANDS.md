...
| `/process [task]` | 🖥️ | **discover a process, then tools for it** — research how this craft does the work well, draft the steps with their why, let the owner cut/add/reorder, then find a skill/MCP per step by function (broadening on empty). Run whenever a task's process isn't obvious; the design flow uses it. PLAYBOOKS |
