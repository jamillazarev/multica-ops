...
| `/mops audience` (`/mops personas`) | ⇆ | audience work: segments, ICP, personas as documents (`docs/audience/`, `templates/PERSONA-template.md`) — and, if the Personas module is on, matching agents. **Infers the creation mode** from the task (docs-only default → per-segment → separate → squad), **stages each persona proto** (a marked guess) **or validated** (interview transcript / QDA distillation), and **asks whether a round has live participants** (their reactions count as facts, not hypotheses). Built/refreshed from research; used by design, marketing and `/mops validate`. Full theatre: MODULES → Persona theatre |
| `/mops validate <what>` | ⇆ | run an artifact past the validators: **Experts squad** gives an evidence-backed verdict (spec, architecture, pricing, roadmap), **Personas squad** reacts as the audience (mockup walkthrough, copy, onboarding). **The verdict format is fixed:** synthetic reactions (hypotheses) and live reactions (facts) are **two separate blocks, never merged into one tally**, and a synthetic verdict states **direction and which bias fired — never magnitude** (no "23% would churn"; synthetics miss extremes — MODULES → Persona theatre). **Findings are weighted by evidence tier** — a live reaction outweighs a twin's, a twin's outweighs a pooled-validated's, a proto's is a marked guess (the same hypothesis-vs-fact ladder the provenance rules carry, applied inside the verdict). **The verdict then returns the gate**, structurally: findings → **a recommendation explicitly labeled as Mops's own judgement, with its reasoning** ("my read: the three findings argue against shipping as-is — a judgement call, not a measurement") → **the gate line — whose decision it is** (per the owner's control level) **and the options**. Baited with "should we ship?", Mops gives its labeled read and **hands the decision back**: it never issues a ship/no-ship verdict, and synthetic findings are **proposals, never numbered shipping requirements**. This keeps *useful over agreeable* (an opinion is owed) and gate ownership (the decision is returned) at once. Neither squad exists? Say so and offer to enable them (`/mops module experts on`, `/mops module personas on` — hires the lineup with your confirmation) |
...
| Command | Where | Routes to |
|---|---|---|
| `/mops team` | ⇆ | roster: agents, roles, models, squads, who is on what — **🎭 theatre entities are excluded from headcount** (a persona is not a hire; MODULES → Persona theatre) |
...
| `/mops upgrade [skill\|all]` | 🖥️ | **the one command for getting current** (no slash commands on this install — an old version or a `skills.sh` install? just say *"upgrade"*, which works anywhere) — four layers: this skill's bytes on your machine (**Mops runs the update itself; content applies on next read** — a restart is only for a new or renamed command or a hook, which register at session start, and only in Claude Code), the workspace migration, imported skills, and the CLI locally and on runtimes — **reported, not run: Mops tells you what's newer and the line to run** (and on a self-hosted server the update is yours, server first), **and only when `active_task_count` is 0**. Update = new bytes; upgrade = your workspace moves to them. Diagram: WORKFLOW. Then: **re-screen the new version first** (an upgrade ships unreviewed third-party code and instructions — diff it against the version you screened, and read what changed in the prose, not only the scripts) → dry-run impact report (skill + dependent agents/squads/autopilots/guide) → commit current to `docs/skill-backups/` (git = history; pre-upgrade SHA logged in `UPGRADES.md`) → apply → reconcile → verify/rollback |
