...
Enabled at the interview or later via `/multica-ops:mops module`. **Design system & brand** come from
...
**Run `/multica-ops:mops process` before drawing anything.** Design is the clearest case for
...
- **A system has three origins — build · adopt · inherit.** Building your own is the
  ...
  treat it like `/multica-ops:upgrade`: preview the diff and its impact on the delta layer before
  applying. **Inheriting an existing own system** (typical at `/multica-ops:join`): audit-and-prepare
...
metaphor boards — are **discovery input** that feeds the book (run via `/multica-ops:mops discovery` /
`/multica-ops:mops research`), then gets distilled; they are not the book.

**Flow (`/multica-ops:mops brand`).** New brand → brand discovery (research + the artifacts above; Brand
...
brand** (typical at `/multica-ops:join`) → audit first: inventory logo/palette/type/voice/
...
- **Proto-persona** — a **pre-interview hypothesis**: cheap, written as a guess, **labelled as
  such**. A decision leaning on one is a decision on `unknown`, not fact (the evidence rungs,
  SKILL). Starting discovery or audience work with no data → offer proto-personas, so the
...
- **The programmatic consequence, stated plainly:** `/multica-ops:mops team` and any staff view **exclude
  ...
  theatre runs to a theatre line**, never to staff; `/multica-ops:status` may carry theatre activity as
...
1. **Reaction provenance is mandatory — synthetic ≠ data.** A synthetic reaction is a
   **hypothesis**, a live person's is a **fact**; `/multica-ops:mops validate` **counts them separately** and
   never merges them into "5 of 7 approved" (COMMANDS → `/multica-ops:mops validate`).
...
format and its two provenance blocks live in COMMANDS → `/multica-ops:mops validate`. **Findings are weighted
...
reworking the theatre**. Two lanes, both feeding `/multica-ops:mops validate`:
...
- **shipped-flow improvement mining** — **once a captured flow exists**, a persona walks the
  ...
  purpose: **capturing shipped flows is not a capability that ships in this release**.
...
- **One-off migration** — the backlog moves here and the old tracker is archived. This is
  `/multica-ops:import`, it ends, and afterwards there is one source of truth.
...
- **Triage before polish.** A dead backlog doesn't deserve a rewrite; the first question is
  *does this still matter*, and the four dispositions apply (accept · decline with a reason ·
  duplicate · snooze — COMMANDS → `/multica-ops:mops feedback`). Dropping is a legitimate answer that
  costs nothing, and a queue where nothing is ever declined is a backlog wearing a different
  label.
...
Run it **after the import creates the issues** (`/multica-ops:import` Pass 3 — they land
