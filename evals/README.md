...
| **isolation** | a **test workspace of its own**, and no reach into the owner's real ones. The console acts on one workspace at a time, so **confirm which one before dispatching**. **And an isolated config dir holding only the skill under test** — `CLAUDE_CONFIG_DIR=<clean>` with `--plugin-dir <corpus copy>`, the shape the sibling project's dispatcher already uses | a player that can see a real company reports on it, and nothing in the transcript marks that it did. Measured 2026-08-02: run under the author's own config, a player answered *"what's next?"* by opening **a different operations skill installed on the same machine** and ran that one's takeover flow instead — the run scored nothing and looked like a result |
...
| **cleanup** | **everything the run created, and the verb differs per entity** — from the console, issues have no `delete` at all (`update --status cancelled` is the whole answer, though a person can delete one in the UI); agents and properties **archive**, reversibly, and archiving a property is what frees a slot against the cap of 20; projects, skills, labels and autopilots **delete** — and a live schedule left behind fires next week. **Say which of these you did**, because cancelled is not gone | a probe left in a workspace is indistinguishable from real work a month later, and a forgotten autopilot spends quota on nobody's behalf |
